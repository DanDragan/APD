...
int main(int argc, char **argv) {
    ...
    int forum[N][N], colors[parties], newforum[N][N], senators[N], newsenators[N];
    ...
    for(week = 0; week < no_weeks; week++) {
        ...
        // paralelizam scanarea fiecarui element al matricii
        #pragma omp parallel for private(j, k, l, x, colors)  
        for(i = 0; i < N; i++) {
            for(j = 0; j < N; j++) {
